...
const Analytics = () => {
  const { admin } = useAuth();
  const [searchParams, setSearchParams] = useSearchParams(); // Change to setSearchParams
  ...
  const handleTabChange = (value: string) => {
    ...
    navigate(`/dashboard/analytics?view=${value}`);
  };
  ...
  const fetchOrganizationAnalytics = async () => {
    try {
      ...
      const adminsRes = await fetch(`${API_BASE_URL}/admins?org_id=${admin?.organizationId}`, {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* ADD onValueChange HERE - this is the fix */}
