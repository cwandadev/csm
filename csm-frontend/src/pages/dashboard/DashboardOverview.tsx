...
const ChartCardSkeleton = () => (
  <Card className="border border-border/60 bg-card">
    <CardHeader>
      ...
      <div className="h-3 w-48 bg-muted animate-pulse rounded mt-1"></div>
    </CardHeader>
    <CardContent>
...
const RecentMembersSkeleton = () => (
  <Card className="border border-border/60 bg-card">
    ...
    <CardContent className="space-y-3">
      {[1, 2, 3, 4, 5].map((i) => (
        <div key={i} className="flex items-center justify-between rounded-xl border border-border bg-accent/40 p-3">
...
// Helper function to get image URL - FIXED for DashboardOverview
const getImageUrl = (image: string | undefined) => {
  ...
  // For user images (stored in users folder)
  const baseUrl = import.meta.env.VITE_API_URL || 'http://localhost:3000/api';
  const staticBaseUrl = baseUrl.replace('/api', '');
  
  // Check if it's a user image (starts with 'user-')
  if (image.startsWith('user-')) {
    const imageUrl = `${staticBaseUrl}/uploads/users/${image}`;
    console.log('[Dashboard] User image URL:', imageUrl);
    return imageUrl;
  }
...
// Avatar Component with image support - FIXED
const UserAvatar = ({ user, size = "md" }: { user: { firstName: string; lastName: string; image?: string }; size?: "sm" | "md" | "lg" }) => {
  ...
  console.log('[UserAvatar] Image URL for', user.firstName, ':', imageUrl);
