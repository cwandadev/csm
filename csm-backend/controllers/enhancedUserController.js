...
export const getEnhancedUsers = async (req, res) => {
  ...
  u.card_uid, u.backup_code, u.fingerprint_template, u.is_active, u.image, u.gender, u.payment_status,
...
export const createEnhancedUser = async (req, res) => {
  ...
  card_uid, backup_code, fingerprint_template, is_active, image, payment_status, // Add payment_status here
  // Student specific
  ...
  fingerprint_template, is_active, image, payment_status, added_by, created_at, updated_at
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [
  ...
  is_active !== undefined ? (is_active ? 1 : 0) : 1, image || null, payment_status || 'pending', adminId
]
    );
...
export const updateEnhancedUser = async (req, res) => {
  ...
  try {
    ...
    // Update user table - ADDED 'payment_status' to allowed fields
    const userFields = [];
    const userValues = [];
    const allowedUserFields = ['first_name', 'last_name', 'email', 'phone', 'gender', 'country', 'province', 'city', 'card_uid', 'backup_code', 'fingerprint_template', 'is_active', 'image', 'payment_status'];
    ...
    // Update role-specific table based on role (rest of the function remains the same)
