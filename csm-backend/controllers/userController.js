...
// Create user (student or employee) with image support
export const createUser = async (req, res) => {
  ...
  try {
    ...
    // Create user with image (NO password column in users table)
    const [userResult] = await connection.execute(
      `INSERT INTO users (
        organization_id, first_name, last_name, email, phone, 
        role, card_uid, fingerprint_template, backup_code, gender, 
        country, province, city, image, added_by, is_active, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())`,
      [
        req.organizationId, firstName, lastName, email || null, phone || null,
        role, card_uid || null, null, backup_code || null, gender || null, 
        country || null, province || null, city || null, image || null,
        req.adminId
      ]
    );
    
    await NotificationService.userAdded(
      req.organizationId,
      `${firstName} ${lastName}`,
      role,
      req.adminId
