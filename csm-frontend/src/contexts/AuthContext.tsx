...
export interface Admin {
  ...
  organizationLogo?: string;  // Add this line
  isVerified: boolean;
  plan: string;
...
export interface RegisterData {
  ...
  addExtraDevice?: boolean;
  extraDeviceFee?: number;
...
const normalizeAdmin = (raw: Record<string, unknown>): Admin => {
  return {
    ...
    organizationLogo: raw.organizationLogo ?? raw.organization_logo ?? "",  // Add this line
...
const register = async (data: RegisterData) => {
  ...
  try {
    if (USE_MOCK) {
      await new Promise((r) => setTimeout(r, 1000));
      const mockAdmin: Admin = {
        id: "2", firstName: data.firstName, lastName: data.lastName, email: data.email,
        username: data.username, profile: "admin_default.jpg", organizationId: "2",
        organizationName: data.orgName, organizationType: data.orgType,
        isVerified: false, plan: data.plan, authProvider: "email",
      };
      persistAdmin(mockAdmin, "mock-token");
      return mockAdmin;
    } else {
      const payload: Record<string, unknown> = {
        ...
        addExtraDevice: data.addExtraDevice || false,
        extraDeviceFee: data.extraDeviceFee || 0,
        planDetails: data.planDetails || {},
      };
      
      // Always include password for email registration
      if (data.password) {
        payload.password = data.password;
      } else if (!data.isGoogleSignup) {
        throw new Error("Password is required for email registration");
      }
      
      console.log("Register payload being sent to API:", { 
        ...payload, 
